...
static const ull FK[4] = {0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};
...
void keyExtention(uchar K[32], uint RK[32]) {
    uint tk[36];
    for (int i = 0; i < 4; i++) {
        tk[i] = chr2int(K + 4 * i);
    }
    ...
    for (int i = 0; i < 32; i++) {
        tk[i + 4] = tk[i + 1] ^ tk[i + 2] ^ tk[i + 3] ^ CK[i];
...
void sm4_encrypt(uchar *input, uchar *output, uchar *K) {
    uint RK[32];
    keyExtention(K, RK);
    uint X[36];
    for (int i = 0; i < 4; i++) {
...
void sm4_decrypt(uchar *input, uchar *output, uchar *K) {
    uint RK[32];
    keyExtention(K, RK);
    uint X[36];
    for (int i = 0; i < 4; i++) {
...
int main() {
    uchar input[16] = "SCUCTF{SM4_TEST}";
    uchar key[16] = "SCUCTF{SM4__KEY}";
    uchar output[16];
    sm4(input, output, key);
    for (int i = 0; i < 16; i++) {
        printf("%d, ", output[i]);
    }
    printf("\n");
